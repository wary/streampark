...
@Configuration
@Slf4j
public class OpenIdClient extends OidcClient {

    @Value("${sso.openid.id}")
    private String clientId;

    @Value("${sso.openid.secret}")
    ...
    protected void clientInit() {
        OidcConfiguration configuration = this.getConfiguration();
        if (configuration == null) {
            configuration = new OidcConfiguration();
            this.setConfiguration(configuration);
        }
        this.getConfiguration().setWithState(false);
